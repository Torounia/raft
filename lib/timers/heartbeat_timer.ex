defmodule Raft.HeartbeatTimer do
  ...
  def handle_cast(:start_heartbeat_timer, %{heartbeat_timer: timer, timeout: timeout, timer_start: nil}) do
    {new_timer, new_timer_start} = if timer == nil do
      Logger.debug("No heartbeat timer to reset. Starting new heartbeat timer, time now = #{inspect(Time.utc_now())}")
  ...
  def handle_info(:heartbeat, %{heartbeat_timer: timer, timeout: timeout, timer_start: timer_start }) do
    Logger.debug("Heartbeat timeout for timer: #{inspect(timer)}, timer duration #{inspect(Time.diff(Time.utc_now(), timer_start, :millisecond))} ")
    if timer != nil do
      Process.cancel_timer(timer)
    end
